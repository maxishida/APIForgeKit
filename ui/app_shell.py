...
NAV_ITEMS = (
    ...
    ("Next.js Blueprint", "/blueprint", "account_tree"),
...
def render_shell(active: str, title: str, subtitle: str, content: Callable[[], None]) -> None:
    ...
    metrics = services.repository.metrics() if status["online"] else {"total": 0, "average_score": 0, "latest": None}
    ...
    with ui.header(elevated=False).classes("afk-header").style("height:76px;"):
        with ui.row().classes("w-full items-center justify-between no-wrap"):
            ...
            with ui.row().classes("items-center gap-3 no-wrap"):
                ...
                ui.html(f"<span class='afk-badge'>Tests {metrics['total']}</span>")
                ui.html(f"<span class='afk-badge'>Avg Score {metrics['average_score']}</span>")
                ui.button("Executar teste", icon="play_arrow", on_click=lambda: ui.navigate.to("/lead-lab")).classes("afk-primary-btn")
